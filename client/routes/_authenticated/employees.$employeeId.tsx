...
function EmployeeDetailPage() {
  ...
  const { data: targetRoles, refetch: refetchTargetRoles } = useQuery({
    queryKey: ["user_roles", employeeId],
    queryFn: async () => {
  ...
  async function handlePromote() {
    setPromoting(true);
    try {
      await promoteUserToAdmin(employeeId);
      toast.success(`${profile?.full_name || "Employee"} has been promoted to HR Admin.`);
      refetchTargetRoles();
    } catch (err: any) {
        ...
        <div className="flex flex-wrap items-center gap-5">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2.5">
              ...
              {targetIsAdmin && (
                <span className="rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-semibold text-primary border border-primary/20 flex items-center gap-1">
